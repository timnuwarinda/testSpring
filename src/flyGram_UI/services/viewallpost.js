...
function viewPost() {
	...
	$
			.ajax({
				url : "http://localhost:9595/viewAllPost",
				type : "GET",
				dataType : 'json',
							...
							.each(
									...
									function(i, item) {
										post += '<div class="col-md-4 col-sm-6"> <div class="template-thumb"><a class="viewm" target="_blank" data-toggle="modal" data-target="#imageVideomodal"><div class="img-wrapper"><img src="'
												...
												+ item.content 
												+ '" alt="" class="img-responsive" /><input class="postid" type="hidden" value="'+ item.id +'"></div></a><div class="page-link"><a href="index.html" target="_blank"> </a></div></div></div>';
										count++;
									});
